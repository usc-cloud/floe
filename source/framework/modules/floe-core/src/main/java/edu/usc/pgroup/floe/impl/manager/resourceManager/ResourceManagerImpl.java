...
public class ResourceManagerImpl implements ResourceManager {
    ...
    public AcquireContainerResponse acquireContainers(
        ...
        if (availableContainer == null) {
            ...
            if(!(handler instanceof GopherInfraHandler))
    ...
    public String registerContainer(String containerID, String host) {
        ...
        LocalClusterInfraHandler handler = null;
        if (cont.getavailableResources().getOptionalConfiguration() != null &&
                cont.getavailableResources().getOptionalConfiguration().getParams().
                        containsKey(GopherInfraHandler.PARTITION)) {
            handler = (GopherInfraHandler) InfraHandlerFactory.getInstance("gopher");
            ResourceIdentifier rid = handler.registerResource(cont.getavailableResources(), host);
            cont.setResourceIdentifier(rid);
            return "Container Info";

        } else {
            handler = (LocalClusterInfraHandler) InfraHandlerFactory.getInstance("Local");
        }
        //Resource
        ResourceIdentifier rid = handler.registerResource(cont.getavailableResources(), host);
        cont.setResourceIdentifier(rid);
